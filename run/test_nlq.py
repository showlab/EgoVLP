    ...
    if args.subsample == 'text':
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm.tqdm(data_loader)):
                ...
                if raw_text not in cache_bert_feature.keys():
                    cache_bert_feature[raw_text] = text_embed.cpu().detach()

        np.save(os.path.join(args.save_feats, 'sentence.npy'), cache_bert_feature)
...
if __name__ == '__main__':
    ...
    args.add_argument('--save_feats', default='/apdcephfs/private_qinghonglin/video_dataset/ego4d/benchmark_splits/nlq/egovlp',
