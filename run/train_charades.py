...
def run():
    ...
    trainer = Multi_Trainer_dist_Charades(args, model, loss, metrics, optimizer,
                      config=config,
...
def init_dataloaders(config, module_data):
    ...
    if "type" in config["data_loader"] and "args" in config["data_loader"]:
        ...
        # config['data_loader']['args'] = replace_nested_dict_item(config['data_loader']['args'], 'shuffle', False)
        ...
        for dl_cfg in config['data_loader']:
            ...
            # dl_cfg['args'] = replace_nested_dict_item(dl_cfg['args'], 'shuffle', False)
