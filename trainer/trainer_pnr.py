...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        with tqdma(zip(*self.data_loader), desc=f"Training epoch {epoch}", total=total_iterations) as progress:
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for data in tqdma(dl, desc=f"Validating dl{dl_idx}"):
...
class Multi_Trainer_dist_PNR(Multi_BaseTrainer_dist):
    """
    Trainer class
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data_li in enumerate(zip(*self.data_loader)):
            ...
            for dl_idx, data in enumerate(data_li):
                ...
                data['video'] = data['video'].to(self.device)
                # imple1
                # data['video'] = torch.flatten(data['video'], start_dim=0, end_dim=1).unsqueeze(1)

                data_label = data['labels'].to(self.device)
                data_state = data['state'].to(self.device)
                ...
                with torch.set_grad_enabled(True):
                    ...
                    # imple1
                    # vid_scores = vid_scores.reshape(data['labels'].shape[0], data['labels'].shape[1], -1)

                    vid_scores = self.allgather(vid_scores, self.n_gpu, self.args)
                    data_label = self.allgather(data_label, self.n_gpu, self.args)
                    data_state = self.allgather(data_state, self.n_gpu, self.args)

                    # imple1
                    loss = torch.mean(data_state.T * self.loss(vid_scores.squeeze(dim=-1), torch.argmax(data_label.long(), dim=1)))
    ...
    def _valid_epoch(self, epoch):
        ...
        preds_arr = {x: [] for x in range(len(self.valid_data_loader))}
        labels_arr = {x: [] for x in range(len(self.valid_data_loader))}
        sc_labels_arr = {x: [] for x in range(len(self.valid_data_loader))}
        fps_arr = {x: [] for x in range(len(self.valid_data_loader))}

        parent_start_frames_arr = {x: [] for x in range(len(self.valid_data_loader))}
        parent_end_frames_arr = {x: [] for x in range(len(self.valid_data_loader))}
        parent_pnr_frames_arr = {x: [] for x in range(len(self.valid_data_loader))}

        with torch.no_grad():
            # for validation we switch the nested loop order, because alternate batches not needed...
            # ... and dataloaders can be of different length
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for batch_idx, data in enumerate(tqdm.tqdm(dl)):
                    data['video'] = data['video'].to(self.device)
                    # imple1
                    # data['video'] = torch.flatten(data['video'], start_dim=0, end_dim=1).unsqueeze(1)
                    data_label = data['labels'].to(self.device)
                    data_state = data['state'].to(self.device)
                    data_fps = data['fps'].to(self.device)

                    data_start = data['parent_start_frame'].to(self.device)
                    data_end = data['parent_end_frame'].to(self.device)
                    data_pnr = data['parent_pnr_frame'].to(self.device)
                    ...
                    data_label_all = [torch.zeros_like(data_label) for _ in range(self.n_gpu)]
                    data_state_all = [torch.zeros_like(data_state) for _ in range(self.n_gpu)]
                    data_fps_all = [torch.zeros_like(data_fps) for _ in range(self.n_gpu)]
                    data_start_all = [torch.zeros_like(data_start) for _ in range(self.n_gpu)]
                    data_end_all = [torch.zeros_like(data_end) for _ in range(self.n_gpu)]
                    data_pnr_all = [torch.zeros_like(data_pnr) for _ in range(self.n_gpu)]

                    torch.distributed.all_gather(vid_embed_all, vid_scores)
                    torch.distributed.all_gather(data_label_all, data_label)
                    torch.distributed.all_gather(data_state_all, data_state)
                    torch.distributed.all_gather(data_fps_all, data_fps)
                    torch.distributed.all_gather(data_start_all, data_start)
                    torch.distributed.all_gather(data_end_all, data_end)
                    torch.distributed.all_gather(data_pnr_all, data_pnr)

                    vid_embed_all = torch.cat(vid_embed_all, dim=0)
                    data_label_all = torch.cat(data_label_all, dim=0)
                    data_state_all = torch.cat(data_state_all, dim=0)
                    data_fps_all = torch.cat(data_fps_all, dim=0)
                    data_start_all = torch.cat(data_start_all, dim=0)
                    data_end_all = torch.cat(data_end_all, dim=0)
                    data_pnr_all = torch.cat(data_pnr_all, dim=0)

                    preds_arr[dl_idx].append(vid_embed_all.cpu())
                    labels_arr[dl_idx].append(data_label_all.cpu())
                    sc_labels_arr[dl_idx].append(data_state_all.cpu())
                    fps_arr[dl_idx].append(data_fps_all.cpu())
                    parent_start_frames_arr[dl_idx].append(data_start_all.cpu())
                    parent_end_frames_arr[dl_idx].append(data_end_all.cpu())
                    parent_pnr_frames_arr[dl_idx].append(data_pnr_all.cpu())
        ...
        for dl_idx in range(len(self.valid_data_loader)):
            ...
            preds_arr = torch.cat(preds_arr[dl_idx])
            labels_arr = torch.cat(labels_arr[dl_idx])
            sc_labels_arr = torch.cat(sc_labels_arr[dl_idx])
            fps_arr = torch.cat(fps_arr[dl_idx])
            parent_start_frames_arr = torch.cat(parent_start_frames_arr[dl_idx])
            parent_end_frames_arr = torch.cat(parent_end_frames_arr[dl_idx])
            parent_pnr_frames_arr = torch.cat(parent_pnr_frames_arr[dl_idx])

            for metric in self.metrics:
                metric_name = metric.__name__
                res = metric(preds_arr, labels_arr, sc_labels_arr, fps_arr,
                             parent_start_frames_arr, parent_end_frames_arr, parent_pnr_frames_arr)
                if self.args.rank == 0:
...
def verbose(epoch, metrics, name="TEST"):
    acc = metrics['keyframe_distance']
    msg = f"{name:s} epoch {epoch}, keyframe_distance: {acc:.1f}"
    print(msg)
    return msg
